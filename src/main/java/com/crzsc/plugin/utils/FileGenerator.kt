...
import com.crzsc.plugin.utils.PluginUtils.lowerCaseFirst
...
//            showNotify("assets path is empty")
        ...
        content.append(
            "// 自动生成，请勿修改\n" +
                    "// VSCode 使用方法参考: \"https://github.com/linjonh/flutter-assets-gen.git\"\n" +
    ...
    private fun generateFileMap(
        ...
        if (root.isDirectory) {
            root.children.filter {
                ...
                if (it.isDirectory) {
                    generateFileMap(it, config, map, ignorePath, assetsRoot)
                } else {
                    config(it, regex, basePath, namedWithParent, map, assetsRoot)
                }
            }
        } else {
            config(root, regex, basePath, namedWithParent, map, assetsRoot)
        }
    }
    ...
    private fun config(
        ...
        map: MutableMap<String, String>,
        assetsRoot: VirtualFile
    ) {
        var key = it.nameWithoutExtension.replace(".", "_").toLowCamelCase(regex)///fileName style
        val value = it.path.removePrefix("$basePath/")
        if (namedWithParent) {
            // 构建从 assetsRoot 到文件父目录的完整路径前缀
            val pathParts = mutableListOf<String>()
            var current = it.parent

            // 从文件的父目录向上遍历到 assetsRoot 的父目录
            while (current != null && current.path != assetsRoot.parent?.path) {
                pathParts.add(0, current.name.toLowCamelCase(regex))
                current = current.parent
            }

            // 拼接所有路径部分作为前缀
            if (pathParts.isNotEmpty()) {
                val prefix = pathParts.joinToString("") { part ->
                    part.upperCaseFirst()
                }
                key = "${prefix.lowerCaseFirst()}${key.upperCaseFirst()}"
            }
